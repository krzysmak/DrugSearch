"""Miscellaneous goodies for psycopg2-binary
...
#
# psycopg2-binary is free software: you can redistribute it and/or modify it
...
#
# psycopg2-binary is distributed in the hope that it will be useful, but WITHOUT
...
from psycopg2-binary import extensions as _ext
...
from psycopg2-binary._psycopg import (                             # noqa
...
from psycopg2-binary._json import (                                # noqa
    json, Json, register_json, register_default_json, register_default_jsonb)


# Expose range-related objects
from psycopg2-binary._range import (                               # noqa
...
from psycopg2-binary._ipaddress import register_ipaddress          # noqa
...
class NamedTupleCursor(_cursor):
    ...
    attributes.
        ...
        >>> nt_cur = conn.cursor(cursor_factory=psycopg2-binary.extras.NamedTupleCursor)
        >>> rec = nt_cur.fetchone()
        >>> rec
...
class StopReplication(Exception):
    ...
    `~psycopg2-binary.Error` as occurrence of this exception does not indicate an
    error.
    """
    pass
    ...
    def create_replication_slot(self, slot_name, slot_type=None, output_plugin=None):
        ...
        if slot_type == REPLICATION_LOGICAL:
            if output_plugin is None:
                raise psycopg2-binary.ProgrammingError(
...
def wait_select(conn):
    ...
    `~psycopg2-binary.extensions.set_wait_callback()`. This function uses
    ...
    """
    import select
    from psycopg2-binary.extensions import POLL_OK, POLL_READ, POLL_WRITE
...
def _solve_conn_curs(conn_or_curs):
    ...
    if conn_or_curs is None:
        raise psycopg2-binary.ProgrammingError("no connection or cursor provided")
    ...
    def parse(self, s, cur, _bsdec=_re.compile(r"\\(.)")):
        ...
        for m in self._re_hstore.finditer(s):
            if m is None or m.start() != start:
                raise psycopg2-binary.InterfaceError(
...
def register_hstore(conn_or_curs, globally=False, unicode=False,
    ...
    *oid* parameter, which can be found using a query such as :sql:`SELECT
    ...
    Raise `~psycopg2-binary.ProgrammingError` if the type is not found.
    """
    if oid is None:
...
class CompositeCaster:
    ...
    def parse(self, s, curs):
        ...
        if len(tokens) != len(self.atttypes):
            raise psycopg2-binary.DataError(
...
| \(? ([^",)]+) [,)]                # or an unquoted string
    ...
    def tokenize(self, s):
        ...
        for m in self._re_tokenize.finditer(s):
            if m is None:
                raise psycopg2-binary.InterfaceError(f"can't parse type: {s!r}")
...
def execute_values(cur, sql, argslist, template=None, page_size=100, fetch=False):
    ...
    **not** contain a total result.
    ...
    '''
    from psycopg2-binary.sql import Composable
